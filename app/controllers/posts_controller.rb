class PostsController < ApplicationController
  ...
  # GET /posts.json
  def index
    #We need to check to see if a date has been entered as a parameter
    ...
    #check if a url requests a month
    elsif Date.valid_date? params[:year].to_i, params[:month].to_i, 1
      start_date = Date.parse("1.#{params[:month]}.#{params[:year]}")
      end_date = start_date.end_of_month

  #   #check if a url requests a month - UK date
  # elsif Date.valid_date? params[:month].to_i, params[:year].to_i, 1
  #     start_date = Date.parse("#{params[:year]}.#{params[:month]}.1")
  #     end_date = start_date.end_of_month

    #check if a url requests a year
